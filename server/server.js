...
wss.on("connection", function (ws) {
  ...
  for (var i = 0; i < subscribers.length; i++) {
    if (subscribers[i] != null) broadcastMessage(i, "connected");
  }

  console.log("~~~~~~~~ WELCOME TO SERVER ~~~~~~ s:" + subscribers.length);
  ws.on("message", function (message) {
    console.log("msg: " + message);
    console.log("split", typeof message);
...
function broadcastMessage(clientId, msg) {
  console.log(
    "broadcast:" + msg.split("###")[0] + " length:" + subscribers.length
  );
  for (var i = 0; i < subscribers.length; i++) {
    if (subscribers[i] != null) subscribers[i].send(msg);
  }
}
...
console.log(
  " Browser öffnen und zu Adresse: http://127.0.0.1:" + webPort + " surfen!"
);
